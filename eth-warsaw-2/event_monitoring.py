...
async def setup_event_monitoring(client):
    # Define event handlers (callbacks receive only entity_key parameter)
    def on_created(entity_key):
        print(f"Monitoring -> Entity created: {entity_key}")

    def on_updated(entity_key):
        print(f"Monitoring -> Entity updated: {entity_key}")

    def on_extended(entity_key):
        print(f"Monitoring -> Entity extended: {entity_key}")

    def on_deleted(entity_key):
        print(f"Monitoring -> Entity deleted: {entity_key}")

    # Start watching events (returns unwatch object)
    unwatch = await client.watch_logs(
        label="",
        create_callback=on_created,
        update_callback=on_updated,
        extend_callback=on_extended,
        delete_callback=on_deleted,
    )

    print("Event monitoring started")
    return unwatch  # Call unwatch.unsubscribe() to stop monitoring
...
async def main():
    ...
    subscription = await setup_event_monitoring(client)
    await create_first_entity(client)

    if isinstance(client, GolemBaseClient):
        await subscription.unsubscribe()  # Stop event monitoring
