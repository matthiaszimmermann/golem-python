...
async def setup_event_monitoring(client):
    ...
    def on_created(entity_key):
        print(f"Event monitoring (entity created): {entity_key}")

    def on_updated(entity_key):
        print(f"Event monitoring (entity updated): {entity_key}")

    def on_extended(entity_key):
        print(f"Event monitoring (entity extended): {entity_key}")

    def on_deleted(entity_key):
        print(f"Event monitoring (entity deleted): {entity_key}")

    # Start watching events (returns unwatch object)
    subscription = await client.watch_logs(
        label="",
    ...
    return subscription  # Call unwatch.unsubscribe() to stop monitoring
